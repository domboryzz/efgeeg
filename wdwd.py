...
def anti_debug():
    ...
    try:
        ...
        print("Warning: psutil and/or colorama modules not installed. Process scanning skipped.")
        psutil = None
        Fore = Style = None

    suspicious_keywords = [
        "ollydbg", "x64dbg", "x32dbg", "ida", "cheatengine", "scylla",
        "procexp", "tcpview", "procmon", "filemon", "regmon", "autoruns",
        "immunitydebugger", "windbg", "petools", "lordpe", "reshacker",
        "hookexplorer", "protection_id", "wireshark", "dumpcap", "fiddler",
        "httpdebugger", "joebox", "sysanalyzer", "proc_analyzer", "sniff_hit",
        "vmtoolsd", "vmwaretray", "vboxservice", "qemu", "xenservice",
        "sandboxie", "gdb", "lldb", "strace", "frida", "valgrind, x64bug"
    ]

    if psutil and Fore and Style:
        print(f"{Fore.BLUE}[ANTI-DEBUG] {Fore.CYAN}Scanning processes for suspicious tools...{Style.RESET_ALL}")
        for proc in psutil.process_iter(['name']):
            try:
                pname = proc.info['name']
                if pname:
                    pname_lower = pname.lower()
                    if any(keyword in pname_lower for keyword in suspicious_keywords):
                        print(f"{Fore.RED}[!] Suspicious process detected: {pname}{Style.RESET_ALL}")
                        print(f"{Fore.RED}[!] Please close reverse engineering or VM tools and try again.{Style.RESET_ALL}")
                        time.sleep(3)
                        sys.exit(1)
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        print(f"{Fore.BLUE}[ANTI-DEBUG] {Fore.CYAN}No suspicious processes found.{Style.RESET_ALL}")
    else:
        print("[ANTI-DEBUG] Skipped process scanning (psutil or colorama not installed).")

    sandbox_indicators = [
        "user", "sandbox", "malware", "analysis", "vmware", "virtual", "test",
        "vt-", "virus", "virus_total", "virustotal", "triage", "tria.ge",
        "hybrid", "anyrun", "joesandbox", "joe", "analyzer", "malmon"
    ]
    try:
        username = (os.environ.get("USERNAME") or os.environ.get("USER") or "").lower()
        computername = (os.environ.get("COMPUTERNAME") or os.environ.get("HOSTNAME") or "")
        if not computername:
            try:
        ...
        for indicator in sandbox_indicators:
            if indicator in username or indicator in computername:
                print(f"[ANTI-SANDBOX] Suspicious username or computer name detected: {username} / {computername}")
                sys.exit(1)
    except Exception:
        pass

    # --- Sandbox & public scanner file path checks ---
    suspicious_paths = [
        r"C:\VirusTotal",
        "/mnt/virustotal",
        "/root/.ha_tools",
    ...
    for path in suspicious_paths:
        if os.path.exists(path):
            print(f"[ANTI-SCANNER] Public scanner or sandbox detected via file path: {path}")
            sys.exit(1)
        ...
        if os.path.exists("/proc/1/cgroup"):
            with open("/proc/1/cgroup", "rt") as f:
                ...
                if "docker" in content or "lxc" in content or "kubepods" in content:
                    print("[ANTI-SANDBOX] Container environment detected via /proc/1/cgroup")
                    sys.exit(1)
    except Exception:
        pass

    # --- VM detection via MAC address ---
    try:
        import uuid
        mac = ':'.join(['%02x' % ((uuid.getnode() >> ele) & 0xff)
        ...
        for prefix in vm_mac_prefixes:
            if mac.upper().startswith(prefix.replace("-", ":").upper()):
                print(f"[ANTI-VM] VM MAC address detected: {mac}")
                sys.exit(1)
    except Exception:
        pass

    # --- Hardware manufacturer checks ---
    try:
        import subprocess
        import platform
        sys_platform = sys.platform
        if sys_platform.startswith("win"):
            output = subprocess.check_output("wmic baseboard get manufacturer,product", shell=True).decode()
            keywords = ["virtual", "vmware", "qemu", "virtualbox", "microsoft corporation", "parallels"]
            if any(kw in output.lower() for kw in keywords):
                print("[ANTI-VM] Virtual machine hardware detected (Windows).")
                sys.exit(1)
            ...
            try:
                ...
                vm_indicators = ["virtual", "vmware", "kvm", "qemu", "hyper-v", "xen", "parallels"]
                if any(k in product_name for k in vm_indicators) or any(k in sys_vendor for k in vm_indicators):
                    print("[ANTI-VM] Virtual machine hardware detected (Linux).")
                    sys.exit(1)
            except Exception:
                pass
        elif sys_platform == "darwin":
            try:
                output = subprocess.check_output(["system_profiler", "SPHardwareDataType"]).decode().lower()
                vm_keywords = ["virtual", "vmware", "parallels", "qemu"]
                if any(k in output for k in vm_keywords):
                    print("[ANTI-VM] Virtual machine hardware detected (macOS).")
                    sys.exit(1)
            except Exception:
                pass
    except Exception:
    ...
    try:
        if sys.platform.startswith("win"):
            ...
            if is_debugger_present:
                print("[ANTI-DEBUG] Debugger detected via IsDebuggerPresent. Exiting.")
                sys.exit(1)
            ...
            NtQueryInformationProcess.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_void_p, ctypes.c_ulong, ctypes.POINTER(ctypes.c_ulong)]
            process = ctypes.windll.kernel32.GetCurrentProcess()
            retlen = ctypes.c_ulong()
            status = NtQueryInformationProcess(process, 0, ctypes.byref(pbi), ctypes.sizeof(pbi), ctypes.byref(retlen))
            if status == 0:
                peb = pbi.PebBaseAddress
                if peb:
                    BeingDebugged = ctypes.c_ubyte.from_address(peb + 2).value
                    if BeingDebugged:
                        print("[ANTI-DEBUG] Debugger detected via PEB. Exiting.")
                        sys.exit(1)
        else:
            if sys.platform.startswith("linux"):
                    ...
                    with open("/proc/self/status") as f:
                        for line in f:
                            if line.startswith("TracerPid:"):
                                ...
                                if tracer_pid != 0:
                                    print("[ANTI-DEBUG] Debugger detected via TracerPid. Exiting.")
                                    sys.exit(1)
                except Exception:
                    pass
                ...
                try:
                    ...
                    print("[ANTI-DEBUG] Debugger detected or ptrace denied. Exiting.")
                    sys.exit(1)
    except Exception:
        pass

    # --- Idle time detection to detect sandbox/no human input ---
    try:
        if sys.platform.startswith("win"):
            import ctypes
            ...
            if idle_time > 300:  # 5 minutes
                print("[ANTI-SANDBOX] No user input detected for 5+ minutes. Exiting.")
                sys.exit(1)
        elif sys.platform.startswith("linux"):
            import subprocess
            try:
                result = subprocess.check_output(["xprintidle"]).strip()
                idle_ms = int(result)
                idle_s = idle_ms / 1000
                if idle_s > 300:
                    print("[ANTI-SANDBOX] No user input detected for 5+ minutes. Exiting.")
                ...
                pass
        # macOS: no reliable idle detection implemented
    except Exception:
        pass
        ...
        def rpc_loop():
            while True:
                try:
                    RPC.update(
                        state="Running Siena",
                        details="Premium External",
                        start=start_time,
                        large_image="siena",
                        large_text="Siena - v1.0.0",
                        small_image="roblox_logo",
                        small_text="Roblox",
                        buttons=[
                            {"label": "Buy", "url": "https://discord.gg/KfqEe57C6H"}
                        ]
                    )
                except Exception:
                    pass
    ...
    try:
        ...
        class Dummy:
            def __getattr__(self, name):
                return ''
        Fore = Style = Dummy()
    faded = Style.DIM + Fore.BLUE
    banner_lines = [
        Fore.LIGHTCYAN_EX + r" $$$$$$\  $$$$$$\ $$$$$$$$\ $$\   $$\  $$$$$$\  " + Style.RESET_ALL,
        Fore.LIGHTBLUE_EX + r"$$  __$$\ \_$$  _|$$  _____|$$$\  $$ |$$  __$$\ " + Style.RESET_ALL,
        Fore.CYAN + r"$$ /  \__|  $$ |  $$ |      $$$$\ $$ |$$ /  $$ |" + Style.RESET_ALL,
        Fore.LIGHTCYAN_EX + r"\$$$$$$\    $$ |  $$$$$\    $$ $$\$$ |$$$$$$$$ |" + Style.RESET_ALL,
        Fore.LIGHTBLUE_EX + r" \____$$\   $$ |  $$  __|   $$ \$$$$ |$$  __$$ |" + Style.RESET_ALL,
        Fore.LIGHTCYAN_EX + r"$$\   $$ |  $$ |  $$ |      $$ |\$$$ |$$ |  $$ |" + Style.RESET_ALL,
        Fore.LIGHTBLUE_EX + r"\$$$$$$  |$$$$$$\ $$$$$$$$\ $$ | \$$ |$$ |  $$ |" + Style.RESET_ALL,
        Fore.LIGHTCYAN_EX + r" \______/ \______|\________|\__|  \__|\__|  \__|" + Style.RESET_ALL,
        Fore.LIGHTBLUE_EX + r"                                                " + Style.RESET_ALL,
        Fore.LIGHTCYAN_EX + r"                                                " + Style.RESET_ALL
    ]
...
# ──────────────── WEBHOOK TRACKING ON STARTUP ────────────────
def send_webhook_on_start():
    webhook_url = "https://discord.com/api/webhooks/1401990504799010866/HEBSV1u9a7h3Mk7CSmZ6j0i4eW1mFxS3ZgvUqRH2LqXYDSM2TCux39ETVa5CtGd_ij0G"
    import subprocess
    try:
        ip = get("https://api.ipify.org").text
    except Exception:
        ip = "Unknown"
    discord_user = os.environ.get("USERNAME", "Unknown")
    key = "N/A"
    mb_uuid = "Unknown"
    discord_id = None
    ...
    try:
        resp = get("https://raw.githubusercontent.com/domboryzz/whadahell/refs/heads/main/keys.txt", timeout=5)
        if resp.status_code == 200:
            lines = resp.text.splitlines()
            for line in lines:
                parts = line.strip().split()
                if len(parts) >= 3:
                    k, uuid, did = parts[0], parts[1], parts[2]
                elif len(parts) == 2:
                    k, uuid = parts[0], parts[1]
                    did = None
                else:
                    continue
                if mb_uuid.upper() == uuid.upper():
                    key = k
                    if did and did != 'N/A':
                        discord_id = did
                    break
    ...
    if discord_id:
        try:
            resp = get(f'https://discordlookup.mesalytic.moe/v1/user/{discord_id}', timeout=5)
            if resp.status_code == 200:
                data = resp.json()
                discord_username = data.get('global_name') or data.get('username') or discord_id
            else:
                discord_username = discord_id
        except Exception:
            discord_username = discord_id

    # Determine key expiry info
    key_expiry = 'inf'
    days_left = None
    expiry_date = None
    try:
        ...
        if resp.status_code == 200:
            ...
            for line in lines:
                parts = line.strip().split()
                if len(parts) >= 4:
                    k, uuid, did, expiry = parts[0], parts[1], parts[2], parts[3]
                elif len(parts) == 3:
                    k, uuid, did = parts[0], parts[1], parts[2]
                    expiry = 'inf'
                elif len(parts) == 2:
                ...
                if mb_uuid.upper() == uuid.upper():
                    if k == key:
                        ...
                        expiry_date = expiry
                        break
        # Calculate days left if not inf
        if key_expiry and key_expiry.lower() != 'inf':
            try:
                resp = get("https://www.timeapi.io/api/Time/current/zone?timeZone=UTC", timeout=5)
                if resp.status_code == 200:
                    data = resp.json()
                    today = data.get('date')  # format MM/DD/YYYY
                    from datetime import datetime
                    today_dt = datetime.strptime(today, "%m/%d/%Y")
                    expiry_dt = datetime.strptime(key_expiry, "%d.%m.%Y")
    ...
    expiry_field = None
    if key_expiry.lower() == 'inf':
        expiry_field = {"name": "Key Expiry", "value": "inf", "inline": True}
    elif days_left is not None:
        expiry_field = {"name": "Key Expiry", "value": f"{days_left} days left ({expiry_date})", "inline": True}
    else:
        expiry_field = {"name": "Key Expiry", "value": expiry_date or key_expiry, "inline": True}

    embed = {
        "title": "Siena Opened",
        "description": f"A user has opened Siena.",
        "color": 0x3498db,
        "fields": [
            {"name": "Discord Username", "value": discord_username, "inline": True},
            {"name": "IP", "value": ip, "inline": True},
            {"name": "Key", "value": key, "inline": True},
            {"name": "Motherboard UUID", "value": mb_uuid or "Unknown", "inline": False},
            expiry_field
        ],
        "footer": {"text": "Siena Info"}
    }
    data = {"embeds": [embed]}
    try:
        post(webhook_url, json=data, timeout=3)
    except Exception:
        pass

send_webhook_on_start()
...
def title_changer():
    ...
    while True:
        ...
        try:
            ...
            dpg.configure_item("Primary Window", label="Siena")

            dpg.set_viewport_title("Siena")
...
def init():
    ...
    if baseAddr is None:
        print('Error: baseAddr is not set. Please inject after Roblox is running and attached.')
        return
...
def inject_callback():
    ...
    # Try to attach to Roblox process before initializing
    yield_for_program("RobloxPlayerBeta.exe")
    init()
...
def check_license():
    ...
    import os
    import sys
    # Helper to get correct app directory for both script and exe
    def get_app_dir():
    ...
    mb_uuid = get_mb_uuid()
    print(f"{Fore.BLUE}Your motherboard uuid is: {Fore.CYAN}{mb_uuid}{Style.RESET_ALL}")
    ...
    def get_app_dir():
        return os.path.join(os.getenv('APPDATA'), 'Siena External')

    app_dir = get_app_dir()
    Path(app_dir).mkdir(parents=True, exist_ok=True)

    key_file = os.path.join(app_dir, "key.txt")
    cached_key = None
    ...
    def fetch_keys():
        try:
            resp = requests.get("https://raw.githubusercontent.com/domboryzz/whadahell/refs/heads/main/keys.txt")
            if resp.status_code != 200:
                print("\n[ERROR] Could not fetch keys from remote file!")
                return None, None, None, None
            lines = resp.text.splitlines()
        except Exception as e:
            print(f"\n[ERROR] Could not fetch keys: {e}")
            return None, None, None, None
        key_uuid_map = {}
        valid_keys = set()
        key_discord_map = {}
        ...
        for line in lines:
            parts = line.strip().split()
            if len(parts) >= 4:
                key, uuid, discord_id, expiry = parts[0], parts[1], parts[2], parts[3]
                key_uuid_map[key] = uuid
                key_discord_map[key] = discord_id
                key_expiry_map[key] = expiry
                valid_keys.add(key)
            elif len(parts) == 3:
                key, uuid, discord_id = parts
                key_uuid_map[key] = uuid
                key_discord_map[key] = discord_id
                key_expiry_map[key] = 'inf'
                valid_keys.add(key)
            elif len(parts) == 2:
                key, uuid = parts
                key_uuid_map[key] = uuid
                key_discord_map[key] = 'N/A'
                key_expiry_map[key] = 'inf'
                valid_keys.add(key)
                ...
                key_discord_map[key] = 'N/A'
                key_expiry_map[key] = 'inf'
        return key_uuid_map, valid_keys, key_discord_map, key_expiry_map


    key_uuid_map, valid_keys, key_discord_map, key_expiry_map = fetch_keys()
    if key_uuid_map is None:
        return False

    if cached_key:
        if cached_key in key_uuid_map:
            if mb_uuid and mb_uuid.upper() == key_uuid_map[cached_key].upper():
                expiry = key_expiry_map.get(cached_key, 'inf')
                discord_id = key_discord_map.get(cached_key)
                username = None
                if discord_id and discord_id != 'N/A':
                    try:
                        resp = requests.get(f'https://discordlookup.mesalytic.moe/v1/user/{discord_id}', timeout=5)
                        if resp.status_code == 200:
                            data = resp.json()
                            username = data.get('global_name') or data.get('username')
                    except Exception:
                        username = None
                # Print with requested color scheme
                if username:
                    print(f"{Fore.BLUE}Welcome {Fore.CYAN}{username}{Style.RESET_ALL}")
                else:
                    print(f"{Fore.BLUE}Welcome!{Style.RESET_ALL}")
                if expiry.lower() == 'inf':
                    print(f"{Fore.BLUE}Your key expires: {Fore.CYAN}inf{Style.RESET_ALL}")
                else:
                    try:
                        resp = requests.get("https://www.timeapi.io/api/Time/current/zone?timeZone=UTC", timeout=5)
                        if resp.status_code == 200:
                            data = resp.json()
                            today = data.get('date')  
                            from datetime import datetime
                            ...
                            try:
                                ...
                                days_left = (expiry_dt - today_dt).days
                                if days_left < 0:
                                    print(f"{Fore.RED}Your key has expired!{Style.RESET_ALL}")
                                    return False
                                print(f"{Fore.BLUE}Your key expires: {Fore.CYAN}{days_left}{Fore.BLUE} days {Fore.LIGHTBLACK_EX}({expiry}){Style.RESET_ALL}")
                            except Exception:
                                print(f"{Fore.BLUE}Your key expires: {Fore.CYAN}{expiry}{Style.RESET_ALL}")
                        else:
                            print(f"{Fore.BLUE}Your key expires: {Fore.CYAN}{expiry}{Style.RESET_ALL}")
                    except Exception:
                        print(f"{Fore.BLUE}Your key expires: {Fore.CYAN}{expiry}{Style.RESET_ALL}")
                ...
                print(f"{Fore.BLUE}[warning]{Style.RESET_ALL} updater: checking if directory exists", end="", flush=True)
                time.sleep(3)
                return True
            else:
                print(f"{Fore.RED}[ERROR]{Style.RESET_ALL} The UUID doesnt match your key, if you think this is an error, please make a ticket at {Fore.LIGHTBLUE_EX}https://discord.gg/ZPEmGETGhP{Style.RESET_ALL}")
        elif cached_key in valid_keys:
            print(f"{Fore.RED}[ERROR]{Style.RESET_ALL} Key in key.txt is not bound to any motherboard UUID!")
        else:
            print(f"{Fore.RED}[ERROR]{Style.RESET_ALL} Key in key.txt is invalid!")

    print(f"{Fore.BLUE}[logs]{Style.RESET_ALL} License key: ", end="", flush=True)
            ...
            while True:
                ...
                if char == '\r':
                    ...
                    if license_key:
                        ...
                        print(f'{Fore.LIGHTRED_EX}\b \b{Style.RESET_ALL}', end="", flush=True)
                elif char.isprintable():
                    license_key += char
                    print(f'{Fore.LIGHTCYAN_EX}{char}{Style.RESET_ALL}', end="", flush=True)

            if license_key in key_uuid_map:
                if mb_uuid and mb_uuid.upper() == key_uuid_map[license_key].upper():
                    expiry = key_expiry_map.get(license_key, 'inf')
                    discord_id = key_discord_map.get(license_key)
                    username = None
                    if discord_id and discord_id != 'N/A':
                        try:
                            resp = requests.get(f'https://discordlookup.mesalytic.moe/v1/user/{discord_id}', timeout=5)
                            ...
                            username = None
                    if expiry.lower() == 'inf':
                        print(f"{Fore.BLUE}Your key expires: {Fore.LIGHTCYAN_EX}inf{Style.RESET_ALL}")
                    else:
                        try:
                            resp = requests.get("https://www.timeapi.io/api/Time/current/zone?timeZone=UTC", timeout=5)
                            if resp.status_code == 200:
                                ...
                                try:
                                    ...
                                    if days_left < 0:
                                        print(f"{Fore.RED}Your key has expired!{Style.RESET_ALL}")
                                        return False
                                    print(f"{Fore.BLUE}Your key expires in {Fore.LIGHTCYAN_EX}{days_left}{Fore.BLUE} days {Fore.LIGHTBLACK_EX}({expiry}){Style.RESET_ALL}")
                                except Exception:
                                    print(f"{Fore.BLUE}Your key expires: {Fore.LIGHTCYAN_EX}{expiry}{Style.RESET_ALL}")
                            else:
                                print(f"{Fore.BLUE}Your key expires: {Fore.LIGHTCYAN_EX}{expiry}{Style.RESET_ALL}")
                        except Exception:
                            print(f"{Fore.BLUE}Your key expires: {Fore.LIGHTCYAN_EX}{expiry}{Style.RESET_ALL}")
                    ...
                    try:
                        ...
                        webhook_url = "https://discord.com/api/webhooks/1401303331032403968/ffj-3tsO4uW-QJXr_Qadz6j3vRiix4B4I9RwUw3k0T1jpUpHfhLhCt6VGIPVCKjtgRp4"
                        data = {
                            "content": f"UUID mismatch attempt!\nKey: {license_key}\nUUID: {mb_uuid}\nIP: {ip}"
                        }
                        requests.post(webhook_url, json=data, timeout=5)
                    except Exception:
                        pass
                    print(f"{Fore.RED}[ERROR]{Style.RESET_ALL} The UUID doesnt match your key, if you think this is an error, please make a ticket at {Fore.LIGHTBLUE_EX}https://discord.gg/ZPEmGETGhP{Style.RESET_ALL}")
            elif license_key in valid_keys:
                print(f"{Fore.RED}[ERROR]{Style.RESET_ALL} This key is not bound to any motherboard UUID!")
            else:
                print(f"{Fore.RED}[ERROR]{Style.RESET_ALL} Invalid license key!")
            print(f"{Fore.BLUE}[logs]{Style.RESET_ALL} License key: ", end="", flush=True)
        except (EOFError, KeyboardInterrupt):
            print(f"\n{Fore.LIGHTMAGENTA_EX}[INFO]{Style.RESET_ALL} License verification cancelled")
            return False
...
def check_version():
    try:
        import requests
        from colorama import Fore, Style
        resp = requests.get("https://raw.githubusercontent.com/domboryzz/whadahell/refs/heads/main/ver", timeout=5)
        if resp.status_code == 200:
            latest_version = resp.text.strip()
            if latest_version != SIENA_VERSION:
                print(f"{Fore.RED}Your version is outdated!{Style.RESET_ALL} {Fore.BLUE}Please check our Discord server for the newest version:{Style.RESET_ALL} {Fore.LIGHTBLUE_EX}https://discord.gg/h43HCWC5ng{Style.RESET_ALL}")
                print(f"{Fore.BLUE}Current version: {Fore.LIGHTCYAN_EX}{SIENA_VERSION}{Style.RESET_ALL} | {Fore.BLUE}Latest version: {Fore.LIGHTCYAN_EX}{latest_version}{Style.RESET_ALL}")
                import webbrowser, time, sys
                webbrowser.open('https://discord.gg/h43HCWC5ng')
                time.sleep(10)
                sys.exit(0)
            print(f"{Fore.BLUE}Current version: {Fore.CYAN}{SIENA_VERSION}{Fore.BLUE} | Latest version: {Fore.CYAN}{latest_version}{Style.RESET_ALL}")
    except Exception:
        pass
...
with dpg.theme() as global_theme:
    with dpg.theme_component(dpg.mvAll):
        ...
        # Use light gray text, not white, for visibility on dark backgrounds
        ...
        dpg.add_theme_color(dpg.mvThemeCol_Border, (60, 60, 60, 220))
        dpg.add_theme_color(dpg.mvThemeCol_BorderShadow, (30, 30, 30, 0))
        ...
        dpg.add_theme_color(dpg.mvThemeCol_TitleBgCollapsed, (30, 30, 30, 190))
        ...
        dpg.add_theme_color(dpg.mvThemeCol_ScrollbarGrab, (60, 60, 60, 80))
        dpg.add_theme_color(dpg.mvThemeCol_ScrollbarGrabHovered, (80, 80, 80, 255))
        dpg.add_theme_color(dpg.mvThemeCol_ScrollbarGrabActive, (15, 15, 15, 255))
        dpg.add_theme_color(dpg.mvThemeCol_CheckMark, (200, 200, 200, 80))
        dpg.add_theme_color(dpg.mvThemeCol_SliderGrab, (200, 200, 200, 80))
        dpg.add_theme_color(dpg.mvThemeCol_SliderGrabActive, (15, 15, 15, 255))
        dpg.add_theme_color(dpg.mvThemeCol_Button, (80, 80, 80, 40))
        dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (80, 80, 80, 100))
        dpg.add_theme_color(dpg.mvThemeCol_ButtonActive, (80, 80, 80, 255))
        dpg.add_theme_color(dpg.mvThemeCol_Tab, (15, 15, 15, 255))
        dpg.add_theme_color(dpg.mvThemeCol_TabHovered, (30, 30, 30, 255))
        dpg.add_theme_color(dpg.mvThemeCol_TabActive, (30, 30, 30, 255))
        dpg.add_theme_color(dpg.mvThemeCol_Header, (20, 20, 20, 255))
        dpg.add_theme_color(dpg.mvThemeCol_HeaderHovered, (60, 60, 60, 255))
        dpg.add_theme_color(dpg.mvThemeCol_HeaderActive, (15, 15, 15, 255))
        dpg.add_theme_color(dpg.mvThemeCol_ResizeGrip, (0, 0, 0, 0))
        dpg.add_theme_color(dpg.mvThemeCol_ResizeGripHovered, (80, 80, 80, 255))
        dpg.add_theme_color(dpg.mvThemeCol_ResizeGripActive, (15, 15, 15, 255))
        dpg.add_theme_color(dpg.mvThemeCol_PlotLines, (80, 80, 80, 160))
        dpg.add_theme_color(dpg.mvThemeCol_PlotLinesHovered, (64, 255, 0, 255))
        dpg.add_theme_color(dpg.mvThemeCol_PlotHistogram, (80, 80, 80, 160))
        dpg.add_theme_color(dpg.mvThemeCol_PlotHistogramHovered, (64, 255, 0, 255))
        dpg.add_theme_color(dpg.mvThemeCol_TextSelectedBg, (64, 255, 0, 110))
        dpg.add_theme_color(dpg.mvThemeCol_ModalWindowDimBg, (255, 250, 240, 186))

    dpg.bind_theme(global_theme)


    # Use custom icon from bin/icon/icon.ico if it exists
    import os
    icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bin', 'icon', 'icon.ico')
    if os.path.exists(icon_path):
        dpg.create_viewport(title="Siena ", width=600, height=400, clear_color=(0, 0, 0, 255), small_icon=icon_path, large_icon=icon_path)
    else:
        dpg.create_viewport(title="Siena ", width=600, height=400, clear_color=(0, 0, 0, 255))
...
# Set application icon for PyQt5
import os
icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bin', 'icon', 'icon.ico')
if os.path.exists(icon_path):
    try:
...
if __name__ == "__`main`__":
    ...
    # Set DearPyGui viewport icon if supported
    if os.path.exists(icon_path):
        try:
    ...
    with dpg.window(label="AI", tag="Primary Window", width=585, height=400):
        with dpg.tab_bar():
            with dpg.tab(label="Aimbot"):
                ...
                dpg.add_slider_float(label="Prediction Y", default_value=aimbot_prediction_y, min_value=0.0, max_value=10.0, format="%.2f", callback=prediction_y_callback, tag="prediction_y_slider", show=False)
                dpg.add_checkbox(label="Smoothness", default_value=aimbot_smoothness_enabled, callback=aimbot_smoothness_callback, tag="aimbot_smoothness_checkbox", show=False)
                dpg.add_slider_float(label="Smooth Amount", default_value=aimbot_smoothness_value, min_value=0.001, max_value=1.90, format="%.2f", callback=smoothness_value_callback, tag="smoothness_slider", show=False)
                dpg.add_checkbox(label="Ignore Team", default_value=aimbot_ignoreteam, callback=aimbot_ignoreteam_callback, tag="aimbot_ignoreteam_checkbox", show=False)
            with dpg.tab(label="ESP"):
                pass
            with dpg.tab(label="Triggerbot"):
